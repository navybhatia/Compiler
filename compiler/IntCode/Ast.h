...
{
	public:
  PrintStmtNode(vector<ExprNode*> *alist, int ln=0, int col=0, string fl=""):
	StmtNode(StmtNode::PRINT, ln,col,fl) { arg_list= alist;};
	vector <ExprNode*> *args(){return arg_list;}
	void  print(ostream& os, int indent) const{};
	void  typePrint(ostream& os, int indent) const{};
	const Type* typeCheck(){};
	string generateCode();
	private:
	vector <ExprNode*>* arg_list;
};
/****************************************************************/
